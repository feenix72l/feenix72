...
let location = { ...QUICK_LOCATIONS[0] };
...
function setLoading(isLoading) {
  ...
  if (unitToggle) unitToggle.disabled = isLoading;
  if (locateButton) locateButton.disabled = isLoading;
  if (weatherGrid) weatherGrid.setAttribute('aria-busy', String(isLoading));
}
...
async function searchCity(query) {
  ...
  location = {
    name: result.name,
    country: result.country || result.country_code || 'Unknown location',
...
async function loadWeather() {
  ...
  try {
    ...
    const locationNode = $('locationName');
    if (locationNode) locationNode.textContent = 'Unable to load weather';
  } finally {
    setLoading(false);
  }
}
...
function render() {
  ...
  $('forecastList').innerHTML = daily.time.map((date, index) => {
    const info = codeInfo(daily.weather_code[index]);
    return `<div class="forecast-day ${index === 0 ? 'today' : ''}">
      <h3>${day(date, index)}</h3>
      <div class="day-icon" aria-label="${info[0]}">${info[1]}</div>
      <div class="range">${temp(daily.temperature_2m_max[index])}° <span>${temp(daily.temperature_2m_min[index])}°</span></div>
      <div class="rain">${info[0]}</div>
    </div>`;
  }).join('');

  const currentHour = hourly.time.findIndex((value) => new Date(value) >= new Date());
  const start = currentHour < 0 ? 0 : currentHour;
  $('hourlyList').innerHTML = hourly.time.slice(start, start + 8).map((timeValue, index) => {
    const dataIndex = start + index;
    const info = codeInfo(hourly.weather_code[dataIndex]);
    return `<div class="hour">
      <span>${index === 0 ? 'Now' : time(timeValue)}</span>
      <span class="hour-icon" aria-label="${info[0]}">${info[1]}</span>
      <strong>${temp(hourly.temperature_2m[dataIndex])}°</strong>
      <span>${hourly.precipitation_probability[dataIndex]}%</span>
    </div>`;
  }).join('');

  $('weatherTip').textContent = current.precipitation > 0
    ? 'A little rain is on the way. Keep an umbrella close.'
    : current.temperature_2m > 25
      ? 'Warm skies today. Don’t forget water and sunscreen.'
      : 'Comfortable weather today. A good day to get outside.';
}

function bindQuickLocations() {
  document.querySelectorAll('[data-location]').forEach((button) => {
    button.addEventListener('click', async () => {
      const city = QUICK_LOCATIONS.find((entry) => entry.name === button.dataset.location);
      if (!city) return;
      location = { ...city };
      setMessage('Pulling forecast...');
      await loadWeather();
    });
  });
}
...
$('searchForm').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    setMessage(error.message || 'Could not find that city.', 'error');
  }
});
...
$('locateButton').addEventListener('click', () => {
  if (!navigator.geolocation) {
    setMessage('Location is not available in this browser.', 'error');
    return;
  }
  ...
  navigator.geolocation.getCurrentPosition(
    async ({ coords }) => {
      location = {
        name: 'Your location',
        country: 'Local forecast',
        latitude: coords.latitude,
        longitude: coords.longitude,
      };
      await loadWeather();
    },
    () => setMessage('Please allow location access, or search for a city instead.', 'error'),
